...
def test_name_map_override(multi_params):
    multi_params['modules'][-1]["params"]["variables_names_map"].update( {"atmosphere_water__liquid_equivalent_precipitation_rate":"RAINRATE"} )
    multi_params['modules'][0]["params"]["variables_names_map"].update( {"UU":"WIND_U"} )
    multi = MultiBMI(**multi_params)
    _t = multi.modules[-1].params.name_map["atmosphere_water__liquid_equivalent_precipitation_rate"]
    assert _t == "RAINRATE"
    _t =  multi.modules[0].params.name_map["UU"]
